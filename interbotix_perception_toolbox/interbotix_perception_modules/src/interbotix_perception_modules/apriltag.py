...
class InterbotixAprilTagInterface(object):
    ...
    :param apriltag_ns: the namespace that the AprilTag node and other related
        parameters are in
    :param init_node: whether or not the module should initalize a ROS node; set to
        `False` if a node was already initalized somwhere else
    :param full_img_get_path: absolute path to image file load location
    :param full_img_save_path: absolute path to image file save location
    """
    ...
    def camera_info_cb(self, msg):
        """ROS Subscriber Callback to get the camera info
    ...
    def find_pose(self, ar_tag_name="ar_tag", publish_tf=False):
        """Calculates the AprilTag pose w.r.t. the camera color image frame
        ...
        :details: TF is published to the StaticTransformManager node (in this package)
            as a static transform if no tags are detected, a generic invalid pose is
            returned
        :details: If multiple tags are detected in the same image, only the first
            detection's pose will be returned
        """
        ...
        if len(detections) == 0:
            if self.v: 
                rospy.logwarn((
                    "Could not find " + ar_tag_name + ". Returning a 'zero' Pose. Make "
                    "sure the tag can be seen in the camera output."
                ))
        else:
